...
class StudentFormView(ttk.Frame):
    def __init__(self, master):
        ...
        ttk.name_labal = ttk.Label(self, text= "نام").grid(column= 1, row= 0, sticky= E, padx= 10, pady= 5)

        self.code_entry = ttk.Entry(self)
        self.code_entry.grid(column= 0,row= 1, sticky= EW, padx= 2, pady= 5)
        ttk.code_labal = ttk.Label(self, text= "کد ملی").grid(column= 1, row= 1, sticky= E, padx= 10, pady= 5)

        self.mobile_entry = ttk.Entry(self)
        self.mobile_entry.grid(column= 0,row= 2, sticky= EW, padx= 2, pady= 5)
        ttk.mobile_labal = ttk.Label(self, text= "موبایل").grid(column= 1, row= 2, sticky= E, padx= 10, pady= 5)

        self.password_entry = ttk.Entry(self)
        self.password_entry.grid(column= 0,row= 3, sticky= EW, padx= 2, pady= 5)
        ttk.password_labal = ttk.Label(self, text= "رمز عبور").grid(column= 1, row= 3, sticky= E, padx= 10, pady= 5)
